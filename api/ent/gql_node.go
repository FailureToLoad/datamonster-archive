...
func (c *Client) noder(ctx context.Context, table string, id int) (Noder, error) {
	...
	case settlement.Table:
		...
		if fc := graphql.GetFieldContext(ctx); fc != nil {
			if err := query.collectField(ctx, true, graphql.GetOperationContext(ctx), fc.Field, nil, settlementImplementors...); err != nil {
				return nil, err
			}
		}
		return query.Only(ctx)
	case survivor.Table:
		query := c.Survivor.Query().
			Where(survivor.ID(id))
		if fc := graphql.GetFieldContext(ctx); fc != nil {
			if err := query.collectField(ctx, true, graphql.GetOperationContext(ctx), fc.Field, nil, survivorImplementors...); err != nil {
				return nil, err
			}
		}
		return query.Only(ctx)
...
func (c *Client) noders(ctx context.Context, table string, ids []int) ([]Noder, error) {
	...
	case survivor.Table:
		query := c.Survivor.Query().
			Where(survivor.IDIn(ids...))
		query, err := query.CollectFields(ctx, survivorImplementors...)
		if err != nil {
			return nil, err
		}
		nodes, err := query.All(ctx)
